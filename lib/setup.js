...
async function launchInTerminalWindow(cmdString, file, opts = {}) {
  ...
  // Wrapper script — runs the editor, drops a sentinel on exit, closes Terminal tab.
  const wrapperBody = [
    "#!/bin/bash",
    "set -e",
  ...
  // AppleScript drives Terminal.app. We single-quote the wrapper path inside
  // the AppleScript double-quoted string, so the shell sees `bash 'path'`.
