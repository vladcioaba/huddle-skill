...
export function saveConfig({ cmd, label, kind, waitMethod }) {
  ...
  const config = {
    ...
    editor: {
      ...
      // When `kind: "term"` and the caller has no TTY, optionally spawn a new
      // Terminal.app window (macOS only for v1) and run the editor inside it.
      // Default true for term editors so vim/nvim/nano "just work" from
      // Claude Code's Bash tool / agents / scripts.
      spawn_in_terminal: existing?.editor?.spawn_in_terminal ?? (kind === "term"),
...
const TERMSPAWN_POLL_MS = 500; // poll sentinel every 500ms

// POSIX-shell single-quote a string (handles ' inside).
function shQuote(s) {
  return `'${String(s).replace(/'/g, "'\\''")}'`;
}

// Launch a terminal editor inside a NEW terminal-emulator window (separate from
// the caller's process). Useful when the caller has no TTY of its own (Claude
// Code Bash tool, agents, CI) but the user still wants a terminal editor.
//
// Strategy: write a wrapper script that runs the editor on FILE then touches
// FILE.done. Tell the OS terminal emulator to run that wrapper. Poll for the
// sentinel file. When it appears, the user has closed the editor.
//
// macOS only for v1 (uses Terminal.app via osascript). Returns
// { exitReason: "spawn_error", error: "spawn_in_terminal currently macOS-only" }
// on other platforms — caller falls back to no_tty handling.
async function launchInTerminalWindow(cmdString, file, opts = {}) {
  const start = Date.now();
  const idleKillMs = opts.idleKillMs ?? DEFAULT_IDLE_KILL_MS;
  const sentinel = `${file}.done`;
  const wrapper = `${file}.run.sh`;
  ...
  if (platform() !== "darwin") {
    return {
      ...
      elapsedSeconds: 0,
      error:
        "editor.spawn_in_terminal is currently macOS-only. Set editor.spawn_in_terminal=false to disable, or use a GUI editor.",
      exitReason: "spawn_error",
    };
  }

  // Clean any stale sentinel/wrapper from previous run.
  try { unlinkSync(sentinel); } catch {}
  try { unlinkSync(wrapper); } catch {}

  // Unique tag for the Terminal window — we use it to find and close the
  // window via AppleScript after the editor exits.
  const tag = `huddle-window-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;

  // Wrapper script — sets the Terminal window title (ESC ] 0 ; tag BEL) so we
  // can locate this window later, runs the editor, drops a sentinel on exit.
  // We avoid `set -e` so a wrapper line failing doesn't leave the Terminal
  // hanging at a non-zero exit with the window still visible.
  const wrapperBody = [
    "#!/bin/bash",
    `printf '\\033]0;${tag}\\007'`,
    `${cmdString} ${shQuote(file)}`,
    `touch ${shQuote(sentinel)}`,
    "exit",
    "",
  ].join("\n");
  writeFileSync(wrapper, wrapperBody, { mode: 0o755 });

  // AppleScript drives Terminal.app. Single-quote the wrapper path inside the
  // AppleScript double-quoted string, so the shell sees `bash 'path'`.
  // `activate` raises the new window to the front.
  const appleScript = [
    `tell application "Terminal"`,
    `  activate`,
    `  do script "bash ${shQuote(wrapper).replace(/"/g, '\\"')}"`,
    `end tell`,
  ].join("\n");
  ...
  try {
    ...
    return {
      exitCode: null,
      elapsedSeconds: 0,
      error: `osascript spawn failed: ${e.message}`,
      exitReason: "spawn_error",
    };
  }
  ...
  while (!existsSync(sentinel)) {
    if (Date.now() - start > idleKillMs) {
      try { unlinkSync(wrapper); } catch {}
      return {
        exitCode: null,
        elapsedSeconds: Math.round((Date.now() - start) / 1000),
        error: `terminal-window editor idle ${Math.round(idleKillMs / 60000)} min — gave up waiting`,
        exitReason: "idle_timeout",
      };
    }
    await new Promise((r) => setTimeout(r, TERMSPAWN_POLL_MS));
  }

  try { unlinkSync(sentinel); } catch {}
  try { unlinkSync(wrapper); } catch {}

  // Close the spawned Terminal window via the title tag. `saving no` suppresses
  // the "do you want to close this window with active processes?" dialog.
  const closeScript = [
    `tell application "Terminal"`,
    `  set winList to every window whose name contains "${tag}"`,
  ...
  try {
    await new Promise((resolve) => {
      const child = spawn("osascript", ["-e", closeScript], { stdio: "ignore" });
      child.on("exit", () => resolve());
      child.on("error", () => resolve());
      // Don't let a stuck osascript hold us up indefinitely.
...
export function launchEditor(cmdString, file, opts = {}) {
  ...
  // 1. TTY guard — only fires when caller has no TTY AND spawn_in_terminal is off.
  if (ttyCheck && editorKind === "term") {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      if (opts.spawnInTerminal) {
        // Defer to terminal-emulator-spawn path. Returns its own exitReason.
        return launchInTerminalWindow(cmdString, file, { idleKillMs });
      }
      return Promise.resolve({
        exitCode: null,
        elapsedSeconds: 0,
        error: `terminal editor "${bin}" requires an interactive TTY but stdin/stdout aren't TTYs. Likely launched under Claude Code Bash, an agent, CI, or a pipe. Set editor.spawn_in_terminal=true (macOS) to auto-open a new Terminal window, switch to a GUI editor via /huddle-set-editor, or invoke from a real terminal.`,
        exitReason: "no_tty",
      });
    }
  }
