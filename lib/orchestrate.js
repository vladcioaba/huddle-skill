...
import { registerSession, completeSession, updateSession } from "./state.js";

function markStoppedSafe(id, reason, noState) {
  if (noState) return;
  try {
    updateSession(id, { status: "stopped", stopped_at: new Date().toISOString(), stop_reason: reason });
  } catch {
    // State entry may not exist (early failure) — fine.
  }
}
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    switch (a) {
      ...
      case "--no-state":  opts.noState = true; break;
      default:
...
export async function orchestrate(opts) {
  ...
  // Register session in state index (unless --no-state).
  if (!opts.noState) {
    try {
      registerSession({
        id,
        seq,
        title: opts.title ?? cfg.template?.default_title ?? "Clarifications needed",
      ...
      console.error(`state register warning: ${e.message}`);
    }
  }
  ...
  while (true) {
    iter++;
    const launchResult = await launchEditor(cfg.editor.cmd, file, {
      editorKind: cfg.editor.kind ?? "gui",
      ttyCheck: cfg.editor.tty_check !== false,
      idleWarnMs: cfg.editor.idle_warn_ms,
      idleKillMs: cfg.editor.idle_kill_ms,
      spawnInTerminal: cfg.editor.spawn_in_terminal === true,
    });
    // Distinguish reliability failures from successful editor close.
    if (launchResult.exitReason === "no_tty") {
      markStoppedSafe(id, "no_tty", opts.noState);
      fail(3, launchResult.error);
    }
    if (launchResult.exitReason === "spawn_error") {
      markStoppedSafe(id, "spawn_error", opts.noState);
      fail(3, `editor spawn failed: ${launchResult.error}`);
    }
    if (launchResult.exitReason === "idle_timeout") {
      // Treat as an aborted session — partial answers may still be present.
      stop = true;
      // Try to parse whatever is there before breaking.
    }
  ...
  // Mark session done in state index (unless --no-state).
  if (!opts.noState) {
    try {
      completeSession(id, bundle);
    } catch (e) {
      console.error(`state complete warning: ${e.message}`);
    }
  }
