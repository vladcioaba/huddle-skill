...
function parseArgs(argv) {
  const opts = { questions: null, id: null, seq: 1, out: null, maxIterations: null, noLoop: false };
...
export async function orchestrate(opts) {
  ...
  let content = buildInitial(id, seq, pending);
  ...
  while (true) {
    ...
    content = buildIteration(id, seq, iter + 1, pending, answeredAccum.map((a) => a.qid));
    writeFileSync(file, content);
  }
